---
date: "2024-11-03"
draft: false
title: "1935 Rolls Royce Sedanca De Ville - Black"
description: "Elegant open chauffeur compartment"
icon: "fas fa-gem"
passengers: 4

######################### banner #####################
banner:
  title: "1935 Rolls Royce Sedanca De Ville"
  image: "images/products/01.jpg"
  content: "Experience the unique elegance of our 1935 Sedanca De Ville with its distinctive open chauffeur compartment. This rare body style combines formality with pre-war luxury, perfect for discerning couples."
  button:
    enable: true
    label: "Book This Car"
    link: "contact/"
  background_class: "bg-light"


######################### about this product #####################
about_this_product:
  enable: true
  title: "Rare Sedanca De Ville <br> Body Style"
  content: "The Sedanca De Ville is a distinctive coachwork style where the rear passenger compartment is fully enclosed while the chauffeur's compartment can be opened to the elements - the ultimate in formal motoring."
  background_class: ""

  features:
  - title: "Distinctive Design"
    icon: "fas fa-star"
    content: "**Unique Sedanca De Ville configuration**


    This rare body style represents the height of 1930s formal motoring.

    * Enclosed rear compartment
    * Open chauffeur section
    * Classic black finish
    * Seats 5 passengers
    "

  - title: "Professional Service"
    icon: "fas fa-user-tie"
    content: "**Impeccable chauffeur experience**


    Our uniformed chauffeurs provide white-glove service.

    * Period-appropriate service
    * Red carpet included
    * Complimentary beverages
    * Wedding decorations
    "


######################### product_info #####################
product_info:
  enable: true
  title: "Perfect For <br> Elegant Celebrations"
  content: "This rare Sedanca De Ville provides an unforgettable experience for couples seeking something truly unique for their special day."
  background_class: "bg-light"
  features:
  - image: "images/products/01.jpg"
    content: "##### For Luxury Weddings


    The Sedanca De Ville's formal styling creates stunning photographs and an atmosphere of old-world elegance.

    <br><br>

    ##### For Heritage Events

    Perfect for vintage-themed celebrations and events celebrating the golden age of motoring.
    "

  - image: "images/products/02.jpg"
    content: "##### For Photography


    This distinctive body style provides unique photo opportunities not available with standard saloons.

    <br><br>

    ##### For Collectors' Events

    Ideal for automotive enthusiasts and period-themed celebrations.
    "


######################### CTA #####################
cta:
  enable: true
  title: "Ready to book this rare Sedanca De Ville?"
  content: "This unique vehicle is one of our most distinctive classics. Contact us to check availability."
  background_class: ""
  button:
    enable: true
    label: "Contact Us"
    link: "contact/"

---
